...
*/
    ...
    for(i=0;i<pixels;++i)
    {
        ...
        if( func == MAXRADIUS ){
            ...
            int dx, dy, dz;
            float radius;
            for ( dz = -parami ; dz <= parami; ++dz ) 
            for ( dy = -parami ; dy <= parami; ++dy ) 
            for ( dx = -parami ; dx <= parami; ++dx ) {
              radius = sqrt( dx*dx + dy*dy + dz*dz );
              if( radius > param ) continue;
              x = xo + dx;
              y = yo + dy;
              z = zo + dz;
        ...
        if( func == AVGRADIUS ){
            ...
            zo = i / xosize / yosize;
            int dx, dy, dz, radius;
            for ( dz = -param ; dz <= param; ++dz ) 
            for ( dy = -param ; dy <= param; ++dy ) 
            for ( dx = -param ; dx <= param; ++dx ) {
              radius = sqrt( dx*dx + dy*dy + dz*dz );
              if( radius > param ) continue;
              x = xo + dx;
              y = yo + dy;
